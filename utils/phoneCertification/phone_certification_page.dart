...
import 'package:pock_gym/api/remoteServicePost.dart';
...
class PhoneCertPage extends StatelessWidget {
  ...
  getPhoneInfo(Map<String, dynamic> data) async {
    ...
    var result = await RemoteServicePost.getCertificationPhone();

    var getinfo = await RemoteServicePost.getCertificationPhoneInfo(data['imp_uid'], jsonDecode(result)['response']['access_token']);
    ...
    print('getPhoneInfo data: ${jsonDecode(getinfo)['response']['unique_key']}');
    print('getPhoneInfo data: ${jsonDecode(getinfo)['response']['unique_in_site']}');
    ...
    sc.userProfileInfo.value.userProfile!.phoneNumber = jsonDecode(getinfo)['response']['phone'];
    sc.userProfileInfo.refresh();

    var phoneData = {"phoneNumber": jsonDecode(getinfo)['response']['phone']};
    var infoResult = await RemoteServicePost.updateUser(phoneData);
